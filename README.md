# coh-piah
Exercício resolvido como parte final do curso Introdução à ciência da computação com Python da Universidade de São Paulo (USP) distribuído pelo Coursera. Abaixo a descrição do exercício
...
No nosso caso, o texto  b b não é conhecido, mas temos a assinatura correspondente: a assinatura de um aluno infectado com COH-PIAH. Ou seja, sabemos o valor de  f_{i,b} que é dado como valor de entrada do programa. 

Caso você não esteja acostumado com a notação matemática, podemos destrinchar essa fórmula da seguinte maneira: 

Para cada traço linguístico  i i (tamanho médio da palavra, relação type-token etc.) se quer a diferença entre o valor obtido em cada texto dado ( a a) e o valor típico do texto de uma pessoa infectada ( b b):  f_{i, a} - f_{i, b}
...
Perceba que quanto mais similares  a a e  b b forem, menor  S_{ab} 
